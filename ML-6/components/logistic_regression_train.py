"""逻辑回归分类器 — Car Evaluation 数据集（6 维特征）训练、评估与可视化"""
...
class_names : array-like
    原始类别名称。
**kwargs : dict
    传递给 LogisticRegression 的参数（如 C, max_iter, multi_class 等）。
...
def train_and_evaluate(X_train, X_test, y_train, y_test, class_names, **kwargs):
    ...
    print(f'\n{"="*60}')
    print(f'  逻辑回归分类报告 (C={kwargs.get("C",1.0)}, '
          f'max_iter={kwargs.get("max_iter",100)})')
    print(f'{"="*60}')
...
le : LabelEncoder
    标签编码器。
**kwargs : dict
    传递给 LogisticRegression 的参数（用于交叉验证错误统计）。
"""
def plot_top_features_scatter(X, y_encoded, le, **kwargs):
    ...
    plt.suptitle('Car Evaluation 数据集 — PCA 降维 + 特征贡献', fontsize=14)
    plt.tight_layout()
    plt.show()
...
y_pred : ndarray
    测试集预测结果。
"""
def run_lr_pipeline(X_train, X_test, y_train, y_test, X_full, y_encoded, le, **kwargs):
