"""决策树分类器 — Car Evaluation 数据集（6 维特征）训练、评估与可视化"""
...
y_train, y_test : array-like
    训练/测试标签（已编码为 0/1/2/3）。
class_names : array-like
    原始类别名称。
**kwargs : dict
    传递给 DecisionTreeClassifier 的参数（如 criterion, max_depth, random_state 等）。
...
def train_and_evaluate(X_train, X_test, y_train, y_test, class_names, **kwargs):
    ...
    print(f'\n{"="*60}')
    print(f'  决策树分类报告 (criterion={kwargs.get("criterion","gini")}, '
          f'max_depth={kwargs.get("max_depth","None")})')
    print(f'{"="*60}')
...
accuracy : float
    准确率（用于标题显示）。
"""
def plot_confusion_matrix(y_test, y_pred, class_names, accuracy, **kwargs):
    cm = confusion_matrix(y_test, y_pred)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class_names)
    disp.plot(cmap='Greens', values_format='d')
    plt.title(f'混淆矩阵 — 决策树\n准确率: {accuracy:.2%}' +
              (f' (max_depth={kwargs.get("max_depth","None")})' if kwargs else ''))
    plt.tight_layout()
    plt.show()
...
def plot_pca_decision_boundary(X, y, class_names, **kwargs):
    ...
    plt.title(f'PCA 降维 + 决策树决策边界 (max_depth={kwargs.get("max_depth","None")})\n'
              f'累计解释方差: {pca.explained_variance_ratio_.sum():.1%}  |  '
              f'准确率: {model.score(X_test, y_test):.2%}')
    plt.colorbar(scatter, ticks=[0, 1, 2, 3], label='类别')
    plt.tight_layout()
    plt.show()
...
def plot_correlation_heatmap(X, y_encoded, class_names):
    ...
    plt.title('Car Evaluation 数据集 6 个特征相关性矩阵', fontsize=14)
...
le : LabelEncoder
    标签编码器（用于获取类别名称）。
**kwargs : dict
    传递给 DecisionTreeClassifier 的参数（用于交叉验证错误统计）。
"""
def plot_top_features_scatter(X, y_encoded, le, **kwargs):
    ...
    plt.suptitle('Car Evaluation 数据集 — PCA 降维可视化 + 特征贡献分析', fontsize=14)
    plt.tight_layout()
    plt.show()
...
y_pred : ndarray
    测试集预测结果。
"""
def run_dt_pipeline(X_train, X_test, y_train, y_test, X_full, y_encoded, le, **kwargs):
    model, y_pred = train_and_evaluate(
        X_train, X_test, y_train, y_test, le.classes_, **kwargs
    )
    accuracy = model.score(X_test, y_test)

    plot_confusion_matrix(y_test, y_pred, le.classes_, accuracy, **kwargs)
