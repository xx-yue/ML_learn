# ML-12：Wholesale Customers 批发客户分类与聚类

使用 UCI Wholesale Customers 数据集，对比 **10 种分类算法 + 2 种聚类算法** 的性能与特点。

## 数据集

- 来源：UCI Wholesale Customers 数据集 `data/Wholesale customers data.csv`
- 440 条样本，2 个类别（Channel 1 = Horeca 酒店/餐饮，Channel 2 = Retail 零售）
- 7 个特征：Region、Fresh、Milk、Grocery、Frozen、Detergents_Paper、Delicassen
- 目标：Channel（1/2）→ LabelEncoder 编码为 0/1（二分类）
- **特征量纲差异大**（Fresh ~数千，Region ~1-3），需要 StandardScaler 标准化
...
├── data/
│   └── Wholesale customers data.csv  # 数据集
├── components/
│   ├── __init__.py
│   ├── knn_train.py                  # K-近邻
...
如果不标准化，Fresh（~数千）等大量纲特征会主导 KNN、SVM 等基于距离的模型。
