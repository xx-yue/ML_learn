"""决策树分类器 — Wholesale Customers 数据集（7 维特征）训练、评估与可视化"""
...
def plot_correlation_heatmap(X, y_encoded, class_names):
    ...
    plt.title('Wholesale Customers 数据集 7 个特征相关性矩阵', fontsize=14)
...
def plot_top_features_scatter(X, y_encoded, le, **kwargs):
    ...
    plt.suptitle('Wholesale Customers 数据集 — PCA 降维可视化 + 特征贡献分析', fontsize=14)
    plt.tight_layout()
    plt.show()
