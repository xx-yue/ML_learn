"""Wholesale Customers 批发客户分类 — 主入口（7 维特征，2 类渠道）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'Wholesale customers data.csv')
...
X = df.drop(columns=['Channel'])     # 特征矩阵（440 行 × 7 列）
y = df['Channel']                    # 目标向量（1, 2）

# 标签编码：1→0, 2→1
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Channel 1', 'Channel 2']  # 2 种渠道

# 标准化 — 各特征量纲差异大，KNN 等距离模型必须标准化
