"""SVM 分类器 — WDBC 数据集（30 维特征）训练、评估与可视化"""
...
def plot_feature_importance(model, feature_names):
    if model.kernel == 'linear':
        ...
        print('SVM 使用非线性核，无法直接提取特征重要性。如需特征重要性，请使用线性核或参考其他算法。')
...
def plot_pca_decision_boundary(X, y, class_names, **kwargs):
    ...
    kernel = kwargs.get('kernel','rbf')
    plt.title(f'SVM 决策边界 (kernel={kernel})\n准确率: {model.score(X_test,y_test):.2%}')
...
def run_svm_pipeline(X_train, X_test, y_train, y_test, X_full, y_encoded, le, **kwargs):
    if 'probability' not in kwargs:
        kwargs['probability'] = True  # 二分类可计算 AUC
