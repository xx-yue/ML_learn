...
使用 UCI Wisconsin Diagnostic Breast Cancer (WDBC) 数据集，对比 **10 种分类算法 + 2 种聚类算法** 的性能与特点。与 ML-1/ML-2 的关键区别在于：**30 维高维特征 + 二分类问题 + AUC 评估指标**。

## 数据集

- 来源：UCI WDBC 数据集 `data/wdbc.data`
- 569 条样本，2 个类别（良性 Benign 357 / 恶性 Malignant 212）
- 30 个特征：10 个细胞核特征（半径、纹理、周长、面积、平滑度、紧致度、凹陷度、凹陷点数、对称性、分形维数）× 3 种统计量（均值、标准误、最大值）
...
如果不标准化，面积（~2000）等大量纲特征会主导 KNN、SVM 等基于距离的模型。
...
# 12. 密度峰值聚类（dc_percent 可调）
centers, y_pred = run_dpc_pipeline(X_scaled, y_true=y_encoded, n_clusters=2, dc_percent=2.0)
```
...
## WDBC vs Wine vs Iris 对比

| 对比项 | Iris（ML-1） | Wine（ML-2） | WDBC（ML-3） |
|--------|-------------|-------------|-------------|
| 样本数 | 150 | 178 | **569** |
| 特征数 | 4 | 13 | **30** |
| 类别数 | 3 | 3 | **2** |
...
| 是否需要标准化 | 不需要 | 必须 | **必须** |
| AUC 指标 | 不适用 | 不适用 | **✓** |
| 相关性热力图标注 | ✓（4×4） | ✓（13×13） | ✗（30×30 太密集） |
| DPC 参数 | dc_percent=0.5 | dc_percent=0.1 | dc_percent=2.0 |
| 决策边界配色 | Set1（3色） | Set1（3色） | **coolwarm（2色）** |
