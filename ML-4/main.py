"""玻璃类型识别 — 主入口（Glass 数据集，9 种化学成分，6 类玻璃）"""
import os
...
columns = [
    ...
    'Ba',                       # 8. 钡
    'Fe',                       # 9. 铁
    'Type',                     # 10. 玻璃类型（目标变量，1-6）
]

script_dir = os.path.dirname(__file__)
file_path = os.path.join(script_dir, 'data', 'glass.data')
df = pd.read_csv(file_path, header=None, names=columns)
df = df.drop('id', axis=1)       # 丢弃 ID 列
...
X = df.drop('Type', axis=1)        # 特征矩阵（214 行 × 9 列）
y = df['Type']                     # 目标向量（1-6）

# 标签编码：1→0, 2→1, ..., 6→5
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Type 1', 'Type 2', 'Type 3', 'Type 4', 'Type 5', 'Type 6']  # 6 种玻璃类型

# 标准化 — Glass 特征量纲差异大，KNN 等距离模型必须标准化
