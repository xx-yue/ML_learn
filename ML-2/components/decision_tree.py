"""决策树分类器 — Wine 数据集（13 维特征）训练、评估与可视化"""
...
def train_and_evaluate(X_train, X_test, y_train, y_test, class_names, **kwargs):
    ...
    # 确保类别名称为字符串（Wine 数据 le.classes_ 是 int）
...
def plot_feature_importance(model, feature_names):
    ...
    bars = plt.bar(range(len(importances)), importances[indices],
                   color=['#1b9e77', '#d95f02', '#7570b3', '#e7298a', '#66a61e',
                          ...
                          '#fb9a99', '#e31a1c', '#ff7f00'][:len(importances)])
...
def plot_correlation_heatmap(X, y_encoded, class_names):
    ...
    plt.title('Wine 数据集 13 个特征相关性矩阵', fontsize=14)
...
"""
使用 PCA 降维散点图 + 特征贡献分析，展示最有区分力的特征组合。
...
def plot_top_features_scatter(X, y_encoded, le, **kwargs):
    ...
    plt.suptitle('Wine 数据集 — PCA 降维可视化 + 特征贡献分析', fontsize=14)
    plt.tight_layout()
    plt.show()
