"""葡萄酒成分识别 — 主入口（Wine 数据集，13 种成分，3 类葡萄酒）"""
import os
...
columns = [
    ...
    'malic_acid',               # 2. 苹果酸
    'ash',                      # 3. 灰分
    'alcalinity_of_ash',        # 4. 灰分的碱度
    'magnesium',                # 5. 镁
    'total_phenols',            # 6. 总酚
    'flavanoids',               # 7. 类黄酮
    'nonflavanoid_phenols',     # 8. 非类黄酮酚
    'proanthocyanins',          # 9. 原花青素
    'color_intensity',          # 10. 颜色强度
    'hue',                      # 11. 色调
    'od280_od315',              # 12. OD280/OD315 稀释葡萄酒
    'proline',                  # 13. 脯氨酸
]

script_dir = os.path.dirname(__file__)
file_path = os.path.join(script_dir, 'data', 'wine.data')
...
X = df.drop('class', axis=1)       # 特征矩阵（178 行 × 13 列）
y = df['class']                     # 目标向量（1, 2, 3）

# 标签编码：1→0, 2→1, 3→2
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Class 1', 'Class 2', 'Class 3']  # 3 种意大利葡萄酒品种

# 标准化 — Wine 特征量纲差异大，KNN 等距离模型必须标准化
...
# centers, y_pred_dpc = run_dpc_pipeline(X.values, y_encoded, n_clusters=3, dc_percent=0.1)