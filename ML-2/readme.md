# ML-2：Wine 葡萄酒成分识别与聚类

使用 UCI Wine 数据集，对比 **10 种分类算法 + 2 种聚类算法** 的性能与特点。与 ML-1
鸢尾花的关键区别在于：13 维高维特征 + 必须标准化处理。

## 数据集

- 来源：UCI Wine 数据集 `data/wine.data`
- 178 条样本，3 个类别（意大利同一地区 3 种不同品种的葡萄酒），分布：59 / 71 / 48
- 13 个特征：酒精、苹果酸、灰分、灰分碱度、镁、总酚、类黄酮、非类黄酮酚、原花青素、颜色强度、色调、OD280/OD315、脯氨酸
- **特征量纲差异极大**（酒精 ~13%、脯氨酸 ~1000），必须做 StandardScaler 标准化
...
```python
# 标准化是必须的 —— 13 个特征量纲差异极大
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)
X_scaled = pd.DataFrame(X_scaled, columns=X.columns)
```

如果不标准化，脯氨酸（~200-1700）等大量纲特征会主导 KNN、SVM 等基于距离的模型。
...
run_dpc_pipeline(X.values, y_encoded, n_clusters=3, dc_percent=0.1)
```
...
> 注：13 维特征无法使用 Pairplot，改用 **13×13 相关性热力图** 展示特征间关系。
...
| 对比项 | Iris（ML-1） | Wine（ML-2） |
|--------|-------------|-------------|
| 样本数 | 150 | 178 |
| 特征数 | 4 | **13** |
| 类别数 | 3 | 3 |
| 是否需要标准化 | 不需要（量纲相近） | **必须**（量纲差异极大） |
| Pairplot | 4×4 可用 | 不可用，改用相关性热力图 |
| DPC 参数 | dc_percent=2.0 | dc_percent=0.1 |
