"""决策树分类器 — Online Retail 数据集（5 维客户特征）训练、评估与可视化"""
...
def plot_feature_importance(model, feature_names):
    ...
    bars = plt.bar(range(len(importances)), importances[indices],
                   color=['#1b9e77', '#d95f02', '#7570b3', '#e7298a', '#66a61e'][:len(importances)])
...
def plot_top_features_scatter(X, y_encoded, le, **kwargs):
    ...
    plt.suptitle('Online Retail 数据集 — PCA 降维可视化 + 特征贡献分析', fontsize=14)
    plt.tight_layout()
    plt.show()
