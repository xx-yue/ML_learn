"""KNN 分类器 — Online Retail 数据集（5 维客户特征）训练、评估与可视化"""
...
def plot_confusion_matrix(y_test, y_pred, class_names, accuracy):
    ...
    plt.title(f'混淆矩阵 — KNN (全部 5 个特征, k=5)\n准确率: {accuracy:.2%}')
    plt.tight_layout()
    plt.show()
...
Online Retail 有 5 个特征，通过 PCA 压缩到 2 维后，
...
def plot_pca_decision_boundary(X, y, class_names, n_neighbors=5):
    ...
    plt.title(f'PCA 降维 + KNN 决策边界 (k={n_neighbors}, 全部 5 个特征)\n'
...
Online Retail 有 5 个特征，使用 5×5 相关性热力图展示特征之间的线性关系强度。
"""
