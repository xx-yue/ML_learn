"""Seeds 小麦种子分类 — 主入口（7 维特征，3 类小麦品种）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'seeds_dataset.txt')
...
X = df.drop('class', axis=1)        # 特征矩阵（210 行 × 7 列）
y = df['class']                      # 目标向量（1, 2, 3）

# 标签编码：1→0, 2→1, 3→2
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Kama', 'Rosa', 'Canadian']  # 3 种小麦品种
