# ML-13：Seeds 小麦种子分类与聚类

使用 UCI Seeds 数据集，对比 **10 种分类算法 + 2 种聚类算法** 的性能与特点。

## 数据集

- 来源：UCI Seeds 数据集 `data/seeds_dataset.txt`
- 210 条样本，3 个类别（Kama、Rosa、Canadian 三种小麦品种），分布：70 / 70 / 70
- 7 个特征：area（面积）、perimeter（周长）、compactness（紧凑度）、kernel_length（核长度）、kernel_width（核宽度）、asymmetry_coeff（不对称系数）、groove_length（凹槽长度）
- 目标：class（1/2/3）→ LabelEncoder 编码为 0/1/2
...
├── data/
│   └── seeds_dataset.txt             # 数据集（tab 分隔，无表头）
├── components/
│   ├── __init__.py
│   ├── knn_train.py                  # K-近邻
...
如果不标准化，area（~10-20）与 compactness（~0.8-0.9）等不同量纲特征会影响 KNN、SVM 等基于距离的模型。
...
> 注：三分类使用 Set1 配色，决策边界 colorbar ticks=[0,1,2]。
