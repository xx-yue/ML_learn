"""Online Shoppers Intention 在线购物意图预测 — 主入口（17 个特征，2 分类）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'online_shoppers_intention.csv')
...
X = df.drop('Revenue', axis=1)       # 特征矩阵（12330 行 × 17 列）
y = df['Revenue']                     # 目标向量（0=No Revenue, 1=Revenue）

# 标签编码
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['No Revenue', 'Revenue']  # 2 分类
...
# centers, y_pred_dpc = run_dpc_pipeline(X.values, y_encoded, n_clusters=2, dc_percent=0.1)
