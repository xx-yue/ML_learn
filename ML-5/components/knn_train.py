"""KNN 分类器 — Wine Quality 数据集（11 维特征）训练、评估与可视化"""
...
def plot_confusion_matrix(y_test, y_pred, class_names, accuracy):
    ...
    plt.title(f'混淆矩阵 — KNN (全部 11 个特征, k=5)\n准确率: {accuracy:.2%}')
    plt.tight_layout()
    plt.show()
...
Wine Quality 有 11 个特征，无法直接可视化。通过 PCA 压缩到 2 维后，
...
def plot_pca_decision_boundary(X, y, class_names, n_neighbors=5):
    ...
    plt.title(f'PCA 降维 + KNN 决策边界 (k={n_neighbors}, 全部 11 个特征)\n'
...
Wine Quality 有 11 个特征，用相关性热力图展示特征之间的线性关系强度。
"""
...
Wine Quality 专用流水线，相比 Iris 做了以下调整：
- 用相关性热力图代替 pairplot（11 维特征 pairplot 过于庞大）
- 增加 PCA 特征贡献分析，展示 11 个特征中哪些最有区分力
