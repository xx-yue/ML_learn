"""Abalone 鲍鱼年龄预测 — 主入口（Abalone 数据集，8 个特征，3 类年龄分组）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'abalone.data')
df = pd.read_csv(file_path, names=column_names)

# ==================== 数据预处理 ====================
# Sex 列是分类型（M/F/I），用 LabelEncoder 编码
le_sex = LabelEncoder()
df['Sex'] = le_sex.fit_transform(df['Sex'])

# 将 Rings 分箱为 3 类：0(rings<=8), 1(9-10), 2(rings>=11)
...
X = df.drop('Rings', axis=1)       # 特征矩阵（4177 行 × 8 列）
y = df['Rings']                     # 目标向量（0/1/2）

# 标签编码
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Young (≤8)', 'Middle (9-10)', 'Old (≥11)']  # 3 个年龄分组
