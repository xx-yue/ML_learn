"""混凝土抗压强度分类 — 主入口（Concrete 数据集，8 个特征，3 类）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'Concrete_Data.xls')
...
X = df.drop('Strength', axis=1)       # 特征矩阵（1030 行 × 8 列）
y = df['Strength']                     # 目标向量（0/1/2）

# 标签编码（已是 0/1/2，但保持一致）
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Low', 'Medium', 'High']  # 混凝土抗压强度低/中/高

# 标准化 — Concrete 特征量纲差异大，KNN 等距离模型必须标准化
