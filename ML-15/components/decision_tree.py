"""决策树分类器 — Mammographic Masses 数据集（5 维特征）训练、评估与可视化"""
...
def plot_correlation_heatmap(X, y_encoded, class_names):
    ...
    plt.title('Mammographic Masses 数据集 5 个特征相关性矩阵', fontsize=14)
...
def plot_top_features_scatter(X, y_encoded, le, **kwargs):
    ...
    plt.suptitle('Mammographic Masses 数据集 — PCA 降维 + 特征贡献', fontsize=14)
    plt.tight_layout()
    plt.show()
