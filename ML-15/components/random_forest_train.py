"""随机森林分类器 — Mammographic Masses 数据集（5 维特征）训练、评估与可视化"""
...
class_names : array-like
    原始类别名称。
**kwargs : dict
    传递给 RandomForestClassifier 的参数。
...
def train_and_evaluate(X_train, X_test, y_train, y_test, class_names, **kwargs):
    ...
    print(f'\n{"="*60}')
    print(f'  随机森林分类报告 (n_estimators={kwargs.get("n_estimators",100)})')
    print(f'{"="*60}')
