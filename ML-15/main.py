"""乳腺肿块恶性预测 — 主入口（Mammographic Masses 数据集，5 个特征，2 分类）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'mammographic_masses.data')
...
X = df.drop('severity', axis=1)       # 特征矩阵（5 列）
y = df['severity']                     # 目标向量（0=良性, 1=恶性）

# 标签编码：0→Benign, 1→Malignant（字母序保持一致）
y = y.map({0: 'Benign', 1: 'Malignant'})
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Benign', 'Malignant']  # 良性 / 恶性
