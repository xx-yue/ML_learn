"""KNN 分类器 — Auto MPG 数据集（7 维特征）训练、评估与可视化"""
...
def plot_confusion_matrix(y_test, y_pred, class_names, accuracy):
    ...
    plt.title(f'混淆矩阵 — KNN (全部 7 个特征, k=5)\n准确率: {accuracy:.2%}')
    plt.tight_layout()
    plt.show()
...
Auto MPG 有 7 个特征，无法直接可视化。通过 PCA 压缩到 2 维后，
...
def plot_pca_decision_boundary(X, y, class_names, n_neighbors=5):
    ...
    plt.title(f'PCA 降维 + KNN 决策边界 (k={n_neighbors}, 全部 7 个特征)\n'
...
Auto MPG 有 7 个特征，用相关性热力图展示特征之间的线性关系强度。
"""
...
def plot_top_features_scatter(X, y_encoded, le, n_neighbors=5):
    ...
    plt.suptitle('Auto MPG 数据集 — PCA 降维可视化 + 特征贡献分析', fontsize=14)
    plt.tight_layout()
    plt.show()
