"""Auto MPG 汽车燃油效率分类 — 主入口（Auto MPG 数据集，7 个特征，3 类）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'auto-mpg.data')
column_names = ['mpg', 'cylinders', 'displacement', 'horsepower', 'weight',
                'acceleration', 'model_year', 'origin', 'car_name']
df = pd.read_csv(file_path, delim_whitespace=True, names=column_names)

# ==================== 数据预处理 ====================
# horsepower 列含 '?' 缺失值，转为 NaN 后删除
df['horsepower'] = pd.to_numeric(df['horsepower'], errors='coerce')
df = df.dropna()
# 移除非数值列
df = df.drop('car_name', axis=1)

# 将连续目标 mpg 按三分位数离散为 3 类
...
X = df.drop('mpg', axis=1)            # 特征矩阵（~392 行 × 7 列）
y = df['mpg']                         # 目标向量（0/1/2）

# 标签编码（已是 0/1/2，但保持一致）
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['Low', 'Medium', 'High']  # 燃油效率低/中/高

# 标准化 — Auto MPG 特征量纲差异大（displacement vs weight），必须标准化
