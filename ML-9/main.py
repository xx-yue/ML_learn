"""银行营销预测 — 主入口（Bank Marketing 数据集，20 个特征，二分类）"""
import os
...
file_path = os.path.join(script_dir, 'data', 'bank-additional', 'bank-additional-full.csv')
...
X = df.drop('y', axis=1)       # 特征矩阵（41188 行 × 20 列）
y = df['y']                     # 目标向量（yes/no）

# 标签编码：no→0, yes→1
le = LabelEncoder()
y_encoded = le.fit_transform(y)
class_names = ['No', 'Yes']  # 二分类：客户是否订阅定期存款

# 标准化 — 20 个特征量纲差异大，KNN/SVM 等距离模型必须标准化
...
# # Bank Marketing 去标签跑：X_scaled 是全部特征，y_encoded 是原标签（用于算 ARI）
# run_kmeans_pipeline(X_scaled.values, y_encoded, n_clusters=2)

# ==================== 12. 密度峰值聚类 ====================
# from components.dpc_train import run_dpc_pipeline
# # DPC 在 Bank Marketing 上：dc_percent 调 0.1-1.0 看效果，n_clusters=2
# centers, y_pred_dpc = run_dpc_pipeline(X_scaled.values, y_encoded, n_clusters=2, dc_percent=0.1)
