"""决策树分类器 — Adult Income 数据集（14 维特征）训练、评估与可视化"""
...
def plot_feature_importance(model, feature_names):
    ...
    bars = plt.bar(range(len(importances)), importances[indices],
                   color=['#1b9e77', '#d95f02', '#7570b3', '#e7298a', '#66a61e',
                          ...
                          '#fb9a99', '#e31a1c', '#ff7f00', '#cab2d6'][:len(importances)])
...
def plot_correlation_heatmap(X, y_encoded, class_names):
    ...
    plt.title('Adult Income 数据集 14 个特征相关性矩阵', fontsize=14)
...
def plot_top_features_scatter(X, y_encoded, le, **kwargs):
    ...
    plt.suptitle('Adult Income 数据集 — PCA 降维 + 特征贡献', fontsize=14)
    plt.tight_layout()
    plt.show()
